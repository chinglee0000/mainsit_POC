/**
 * AirdropClaim View - UI Layer (GREEN ZONE)
 * 
 * Pure UI rendering for airdrop claim card.
 * Displays locked, claimable, or claimed states.
 */

import React from 'react';
import { Trophy, Sparkles, ArrowRight, Lock, ChevronRight } from 'lucide-react';
import type { AirdropState } from './index';

// ─── Types ──────────────────────────────────────────────
export interface AirdropClaimViewProps {
  state: AirdropState;
  score: number;
  threshold: number;
  rewardAmount: number;
...
const LockedState: React.FC<{
  ...
  progress: number;
  onGoBack?: () => void;
}> = ({ score, threshold, progress, onGoBack }) => (
...
const ClaimedState: React.FC<{
  ...
  showConfetti: boolean;
  onGoBack?: () => void;
}> = ({ rewardAmount, showConfetti, onGoBack }) => (
  <div className="card animate-fade-in" style={cardStyle}>
...
// ─── Claimable State ────────────────────────────────────
const ClaimableState: React.FC<{
  score: number;
  rewardAmount: number;
  onClaim: () => void;
}> = ({ score, rewardAmount, onClaim }) => (
  <div className="card animate-fade-in" style={cardStyle}>
    <div style={{
      padding: '16px 20px',
      borderBottom: '1px solid rgba(255, 255, 255, 0.06)',
...
// ─── Main Component ─────────────────────────────────────
export const AirdropClaimView: React.FC<AirdropClaimViewProps> = ({
  state,
  score,
  threshold,
  rewardAmount,
  ...
  if (state === 'locked') {
    return <LockedState score={score} threshold={threshold} progress={progress} onGoBack={onGoBack} />;
  }

  if (state === 'claimed') {
    return <ClaimedState rewardAmount={rewardAmount} showConfetti={showConfetti} onGoBack={onGoBack} />;
  }

  return <ClaimableState score={score} rewardAmount={rewardAmount} onClaim={onClaim} />;
};
