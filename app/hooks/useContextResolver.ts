...
import type { URLContext, UserStatus } from '../store/types';

// Flow config helper (simplified version)
function getWelcomeNodeId(contextId: string): string {
  const welcomeNodes: Record<string, string> = {
  ...
  return welcomeNodes[contextId] || 'welcome';
}
...
export function useContextResolver() {
  ...
  useEffect(() => {
    ...
    const welcomeNodeId = getWelcomeNodeId(resolvedContextId);
    setCurrentStep(welcomeNodeId);
